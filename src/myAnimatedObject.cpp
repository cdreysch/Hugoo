...
sf::Vector2f myAnimatedObject::getDrawingPosition(){
	int xd = position.x+dataPtr->getSequencePtr(currentSequenceIndex)->getDispositionAt(currentRectIndex)->x;
	int yd = position.y + dataPtr->getSequencePtr(currentSequenceIndex)->getDispositionAt(currentRectIndex)->y;		 
	return sf::Vector2f(xd,yd); 
}
